...
$id_tequio = isset($_GET['id_tequio']) ? (int)$_GET['id_tequio'] : (isset($_GET['id']) ? (int)$_GET['id'] : 0);

$pageTitle = "Pase de Lista - Tequio";
$activePage = "tequios";
...
// Obtener comuneros y su estatus en este tequio
$query = "
    SELECT c.id_comunero, c.numero_progresivo, c.nombre_completo, COALESCE(ct.cumplio, FALSE) as asistio
    FROM comunero c
    LEFT JOIN cumplimiento_tequio ct ON c.id_comunero = ct.id_comunero AND ct.id_tequio = $id_tequio
    WHERE c.activo = TRUE
...
<head>
    ...
    <title>SysComunal | Pase Lista Tequio</title>
    ...
    <main class="main-wrapper">
        ...
        <div class="content-container animate-fade-in">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
                <div>
                    <h1 style="font-size: 1.75rem; color: var(--text-main); margin-bottom: 0.5rem;">Pase de Lista de Faena</h1>
                    <p style="color: var(--text-muted); font-weight: 500;"><?= date('d/m/Y', strtotime($tequio['fecha'])) ?> - <?= htmlspecialchars($tequio['descripcion']) ?></p>
                </div>
                <div>
